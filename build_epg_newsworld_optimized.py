...
ALIASES = {
    ...
    "France24enEspanol.ar": "France24Espanol.ar",
    "NoticiasTelemundoAHORA.us": "Telemundo.Ahora.us",
...
def download(url, timeout=600):
    ...
    if os.path.exists(cache_path):
        with open(cache_path, "rb") as f:
            data = f.read()
        print(f"  {name}: cache ({len(data):,} bytes)")
        return data
...
def source_match(cid, display_name):
    ...
    for tid in tvg_ids:
        ntid = norm(tid)
        if ntid in nc or nc in ntid:
            return tid
    for tid, base_name in m3u_names.items():
        nb = norm(base_name)
        if nb and (nb in nc or nc in nb):
            return tid
    return None
...
for url in EPG_SOURCES:
    ...
    print(f"    -> +{ch} canais, +{pr} programas (total: {len(matched_ids)}/{len(tvg_ids)})")
...
if prog_hoje > 0 and prog_amanha > 0:
    pct_hoje = len(canais_hoje) * 100 // len(canais) if canais else 0
    pct_amanha = len(canais_amanha) * 100 // len(canais) if canais else 0
    print(f"EPG FUNCIONANDO! {len(matched_ids)}/{len(tvg_ids)} canais com dados, "
          f"{pct_hoje}% com programacao hoje, {pct_amanha}% amanha.")
    sys.exit(0)
else:
