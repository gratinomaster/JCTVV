...
for e in m3u_entries:
    ...
    for m in re.finditer(r'channels/([0-9a-f]{24})', e['logo'] or ''):
        logo_contains_id[m.group(1)] = e['tvg_id']

print(f"  {len(m3u_entries)} canais no M3U: {sorted(tvg_ids)}")
...
def source_match(cid, display_name):
    ...
    if nc in tvg_norm:
        return tvg_norm[nc]
    if cid in logo_contains_id:
        return logo_contains_id[cid]
    if display_name:
...
print(f"  Canais com EPG: {sorted(matched_ids)}")
...
print()
if len(canais) == len(tvg_ids) and prog_hoje > 0 and prog_amanha > 0:
    ...
    sem_amanha = sorted(tvg_ids - canais_amanha)
    if sem_amanha:
        print(f"  Obs: PlutoTV nao publica programacao de amanha para: {sem_amanha}")
    sys.exit(0)
else:
