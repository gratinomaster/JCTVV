...
def main():
    ...
    print(f'Canais: {len(channels)} | URLs: {len(all_urls)}')
    ...
    for ci, ch in enumerate(channels):
        working_urls = []
        for url in ch['urls']:
            ok, _ = results.get((ci, url), (False, 'not_tested'))
            if ok:
        ...
        if working_urls:
            ...
            removed += 1
            removed_names.append(get_channel_name(ch['extinf']))

    with open(M3U_FILE, 'w', encoding='utf-8') as f:
        f.writelines(new_lines)

    print(f'\n--- Resultado ---')
    print(f'Mantidos:  {kept} canais')
