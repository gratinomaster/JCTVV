#!/usr/bin/env python3
"""Test all channels in lista5.m3u, remove dead ones, overwrite lista5.m3u."""
...
def test_url(url, timeout=20):
    ...
    try:
        ...
        out = result.stdout
        m = re.search(r'__HTTP_CODE:(\d+)__\s*$', out)
        code = int(m.group(1)) if m else 0
        body = re.sub(r'__HTTP_CODE:\d+__\s*$', '', out).strip()
...
def main():
    ...
    backup = f'{FILEPATH}.bak.pre_teste_{ts}'
    ...
    print(f"Mantidas: {kept} entradas | Removidas: {removed} entradas")
