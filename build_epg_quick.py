...
EPG_SOURCES = [
    ...
    ("braziltv", "https://fastly.jsdelivr.net/gh/limaalef/BrazilTVEPG@main/epg.xml"),
    ("claro", "https://github.com/limaalef/BrazilTVEPG/raw/refs/heads/main/claro.xml"),
    ("pluto", "https://raw.githubusercontent.com/matthuisman/i.mjh.nz/master/PlutoTV/us.xml"),
]
...
def process_epg_bytes(raw_bytes, source_name):
    ch_count = 0
    pr_count = 0
    id_remap = {}
    try:
...
for name, url in EPG_SOURCES:
    raw = download(url, timeout=600)
    if raw is None:
        continue
    ch, pr = process_epg_bytes(raw, name)
