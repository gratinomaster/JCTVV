...
EPG_SOURCES = [
    ...
    "https://fastly.jsdelivr.net/gh/limaalef/BrazilTVEPG@main/vivoplay.xml",
    "https://fastly.jsdelivr.net/gh/limaalef/BrazilTVEPG@main/globo.xml",
...
if canais_hoje:
    print(f"  Channels with today's data ({len(canais_hoje)}): {sorted(canais_hoje)}")
if canais_amanha:
    print(f"  Channels with tomorrow's data ({len(canais_amanha)}): {sorted(canais_amanha)}")

print()
print("  Sample programmes per channel:")
for ch_id in sorted(all_channels.keys()):
    ch_progs = [p for p in programas if p.get("channel") == ch_id]
    hoje_progs = [p for p in ch_progs if p.get("start", "")[:8] == hoje]
    if hoje_progs:
        titles = [p.findtext('title', '?') for p in hoje_progs[:3]]
        print(f"    {ch_id}: {', '.join(titles)}")

if prog_hoje > 0 and prog_amanha > 0:
    print(f"\n  EPG OK! {prog_hoje} programmes for today, {prog_amanha} for tomorrow.")
    sys.exit(0)
else:
